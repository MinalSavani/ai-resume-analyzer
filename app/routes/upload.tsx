...
if (!(imageFile instanceof File)) {
    setStatusText('Error: Converted image is not a valid File');
    setIsProcessing(false);
    return;
}
...
if (!uploadedImage) {
    ...
    return;
        ...
        try {
            ...
            if (!feedback) {
                setStatusText('Error: Failed to analyze resume - no feedback received.');
                setIsProcessing(false);
                return;
            }
            ...
            if (error && typeof error === 'object' && error.error && typeof error.error.message === 'string') {
                errorMessage = `Analysis failed: ${error.error.message}`;
                    ...
                    {isProcessing ? (
                        <>
                            ...
                            <img src="/images/resume-scan.gif"
                                className='w-full' alt="Resume scanning animation" />
                        </>
                    ) : (
                        <h2>
                            Drop your resume for an ATS and improvement tips
                        </h2>
                    )}
